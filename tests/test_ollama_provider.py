...
async def test_analyse_receipt_from_model_success(mock_client):
    # Arrange
    provider = OllamaProvider(host="http://localhost:11434")
    content = '{"merchant_name": "Test Shop", "merchant_address": "123 Main St", "receipt_number": "REC001", "date": "2024-08-06", "time": "14:00", "currency": "USD", "line_items": [{"description": "Coffee", "quantity": 1, "unit_price": 5.00, "total_price": 5.00, "category": "restaurant"}], "taxes": [{"name": "VAT", "rate": 0.20, "amount": 1.00}], "subtotal": 5.00, "discount_total": 0.00, "tax_total": 1.00, "tip": 0.50, "total": 6.50, "payment_method": "credit_card"}'
...
async def test_analyse_receipt_from_model_retry_empty(mock_client):
    ...
    mock_client.chat.side_effect = [
        ...
        MagicMock(
            message=MagicMock(
                content='{"merchant_name": "Retry Shop", "merchant_address": "123 Retry St", "receipt_number": "REC002", "date": "2024-08-06", "time": "15:00", "currency": "USD", "line_items": [{"description": "Retry Item", "quantity": 1, "unit_price": 1.0, "total_price": 1.0, "category": "other"}], "taxes": [], "subtotal": 1.0, "discount_total": 0.0, "tax_total": 0.0, "tip": 0.0, "total": 1.0, "payment_method": "credit_card"}'
            )
        ),
    ]
